...
const CreateTokenTreat: FC = () => {
  ...
  const checkAndSetAmounts = async () => {
    ...
    console.log(`Total Chargeable Amount: ${ethers.formatUnits(totalAmountinUnits, tokenDecimals)}`)
  }
      ...
      try {
        if(treatToken) {
          ...
          if(checkList.length === 0) {
            ...
            setSelectTokenList([...selectTokenList, {
  ...
  const createTreat = async () => {
    ...
    try {
      ...
      if(tokenContract) {
        const currentAllowance = await tokenContract.allowance(account.address, TOKEN_TREAT_CONTRACT_ADDRESS[chainId]);  
        if (currentAllowance < totalChargeableAmountInUnits) {
          const tx = await tokenContract.approve(tokenTreatContractAddress, totalChargeableAmountInUnits);
          await tx.wait();
        }
      }
      ...
      const metadata =  createMetaData(treatImageHash, treatDescription, treatType, treatValidity, {});
      ...
      if(receipientsAddresses.length == 0) {
        ...
        console.log(tx)
        await tx.wait();
        notifySuccess({title: "Success", message: "Treat created successfully TxHash: " + tx.hash});
      }else {
        notifyError({title: "Error", message: "Not supported yet, please enter only one receipient address"})
      }
  ...
  return (
    ...
    gap={5}>
      ...
      <VStack w={"45%"} minWidth={"270px"} gap={2}>
        <Text textAlign="left" fontWeight="bold">Treat Name</Text>
        <Input
          value={treatName}
      ...
      <VStack w={"45%"} minWidth={"270px"} gap={2} textAlign="left">
        <Text textAlign="left" fontWeight="bold">Treat Type</Text>
        <Select value={treatType} onChange={(e)=> {
      ...
      <VStack w={"45%"} minWidth={"270px"} gap={2} textAlign="left">
        <Text textAlign="left" fontWeight="bold">Treat Value</Text>
        <Input
          value={treatValue}
      ...
      <VStack w={"45%"} minWidth={"270px"} gap={2} textAlign="left">
        <Text textAlign="left" fontWeight="bold">Token Address for Treat</Text>
        <Flex>
        <TokenSelectDropdown
      ...
      <VStack w={"45%"} minWidth={"270px"} gap={2} textAlign="left">
        <Text textAlign="left" fontWeight="bold">Burn On Claim</Text>
        <Select value={burnOnClaims} onChange={(e)=> {
      ...
      <VStack w={"45%"} minWidth={"270px"} gap={2} textAlign="left">
        <Text textAlign="left" fontWeight="bold">Is Transferable</Text>
        <Select value={transferable} onChange={(e)=> {
      ...
      <VStack w={"45%"} minWidth={"270px"} gap={2} textAlign="left">
        <Text textAlign="left" fontWeight="bold">Treat Description</Text>
        <Input
      ...
      <VStack w={"45%"} minWidth={"270px"} gap={2} textAlign="left">
        <Text textAlign="left" fontWeight="bold">Treat Validity</Text>
        <Input
          value={treatValidity}
          onChange={(e) => {
      ...
      <VStack w={"45%"} minWidth={"270px"} gap={2} textAlign="left">
        <Text textAlign="left" fontWeight="bold">Refund Treasury Address</Text>
        <Input
      ...
      <VStack w={"100%"} minWidth={"270px"} gap={2} textAlign="left">
        <Text textAlign="left" fontWeight="bold">Receipinets</Text>
        <Input
          value={receipients}
      ...
      <VStack w={"100%"} minWidth={"270px"} gap={2} textAlign="left">
        {platformFee && totalChargeableAmount && tokenSymbol && (
          <Box p={4} borderWidth={1} borderRadius="lg" borderColor="teal.500" bg="gray.50">
            <Text fontWeight="bold" fontSize="lg" color="teal.700">Invoice:</Text>
            <Text fontWeight="bold" color="teal.600">
              Treat Amount: <Text as="span" color="black">{treatValue} {tokenSymbol}</Text>
            </Text>
            <Text fontWeight="bold" color="teal.600">
              Platform Fee: <Text as="span" color="black">{platformFee} {tokenSymbol}</Text>
            </Text>
            <Text fontWeight="bold" color="teal.600">
              Total Chargeable Amount: <Text as="span" color="black">{totalChargeableAmount} {tokenSymbol}</Text>
            </Text>
          </Box>
        )}
      </VStack>
