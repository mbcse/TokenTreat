...
contract EduTreat is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable {
    ...
    address refundTreasury, bool burnOnClaim, bool transferable, string memory treatMetadata) public payable {
        require(expiry > block.timestamp, "EduTreat: expiry must be in the future");
    ...
    function claimTreat(uint256 tokenId) public {
        require(treatInfo[tokenId].expiry > block.timestamp, "EduTreat: treat expired");
        require(treatInfo[tokenId].status == TreatStatus.ACTIVE, "EduTreat: treat already claimed or expired");
    ...
    function claimTreatAtAddress(uint256 tokenId, address claimAddress) public {
        require(ownerOf(tokenId) == msg.sender, "EduTreat: not owner");
        require(treatInfo[tokenId].expiry > block.timestamp, "EduTreat: treat expired");
        require(treatInfo[tokenId].status == TreatStatus.ACTIVE, "EduTreat: treat already claimed or expired");
        treatInfo[tokenId].status = TreatStatus.CLAIMED;
        if(treatInfo[tokenId].tokenAddress != address(0)) {
    ...
    function burnTreat(uint256 tokenId) public {
        require( block.timestamp > treatInfo[tokenId].expiry, "EduTreat: treat not expired");
        require(treatInfo[tokenId].status == TreatStatus.ACTIVE, "EduTreat: treat already claimed or expired");
        ...
        returns (address)
    {
        ...
        if(!treatInfo[tokenId].transferable) {
            revert("EduTreat: token not transferable");
        }
